...
export default function SwapPage() {
  ...
  const tokenToStxSwap = async (tokenAmount: number) => {
    ...
    const options: ContractCallRegularOptions = {
      ...
      appDetails,
      onFinish: ({ txId }) =>
        addTransactionToast(txId, `Swapping ${tokenAmount} Magic Beans...`),
    }
  ...
  const makeExchangeRatioSection = () => {
    ...
    return (
      <section>
        ...
        <p>
          Current balance: <b>{+exchangeInfo.stxBalance.toFixed(6)}</b> STX and{' '}
          <b>{+exchangeInfo.tokenBalance.toFixed(6)}</b> Magic Beans
        </p>
      </section>
    )
  }
